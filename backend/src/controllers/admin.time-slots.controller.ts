...
export async function createTimeSlot({ body, set }: any) {
...
export async function updateTimeSlot({ params, body, set }: any) {
...
export async function toggleTimeSlotActive({ params, set }: any) {
  const id = Number(params.id);
  if (!Number.isFinite(id) || id <= 0) {
    set.status = 400;
    return { message: "Invalid id" };
...
export async function deleteTimeSlot({ params, query, set }: any) {
  const id = Number(params.id);
  if (!Number.isFinite(id) || id <= 0) {
    set.status = 400;
    return { message: "Invalid id" };
  ...
  try {
    ...
    if (
      ...
      set.status = 409;
      return {
        message: "เธเนเธงเธเน€เธงเธฅเธฒเธเธตเนเธกเธตเธเนเธญเธกเธนเธฅเธญเนเธฒเธเธญเธดเธเธญเธขเธนเน เธเธฃเธธเธ“เธฒเธขเธทเธเธขเธฑเธเธเนเธญเธเธฅเธ",
