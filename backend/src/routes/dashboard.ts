...
export let dashboardRoute = new Elysia({ prefix: "/api/dashboard" })
  .get("/department/:departmentId/summary", async ({ params, request, set }) => {
  ...
  .get("/department/:departmentId/comments", async ({ params, query, request, set }) => {
    const departmentId = Number(params.departmentId);
  ...
  .get(
    "/department/:departmentId/yearly",
...
dashboardRoute = dashboardRoute.post(
  "/qrcode/:departmentId/reset",
  async ({ params, request, set }) => {
    ...
    try {
      ...
      return {
        message: "QR Code หน่วยงานรีเซตสำเร็จ",
        department: {
          id: department.id,
